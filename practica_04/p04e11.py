...
Page 2una palabra existente, utilizando el módulo pattern, con un mínimo de
...
def obtener_palabra_validar(window, limite_extremo_uno, limite_extremo_dos):
    palabra_validar = []

    if obtener_posicion_palabra(limite_extremo_uno, limite_extremo_dos) == 'H':
        for x in range(limite_extremo_uno[0], limite_extremo_dos[0] + 1):
            # Es indistinto si es "x, limite_extremo_uno[1]" o "x, limite_extremo_dos"
            # Ya que el valor de 'y' no cambia
            palabra_validar.append(window[(x, limite_extremo_uno[1])].GetText())
    else:
        for y in range(limite_extremo_uno[1], limite_extremo_dos[1] + 1):
            # Es indistinto si es "limite_extremo_uno[0], y" o "limite_extremo_dos[0], y"
            # Ya que el valor de 'x' no cambia
            palabra_validar.append(window[(limite_extremo_uno[0], y)].GetText())

    return ''.join(palabra_validar)


def validar_palabra(palabra_validar):
    palabra_validar = palabra_validar.lower()
    resultado = None

    if palabra_validar in spelling.keys() and palabra_validar in lexicon.keys():  # Dificultad -> Facil
        resultado = True
...
layout = tablero_a + tablero_b + [[sg.Button('CANCELAR SELECCIÓN', key='cancelar',
                                      ...
                                      sg.Button('VALIDAR PALABRA', key='validar',
                                                font='Helvetica 12 bold', size=(20, 2), pad=(4, 1),
                                                button_color=('white', 'green'), disabled=True)]]

window = sg.Window('Practica_04 | Ejercicio_11', layout, size=(800, 670), background_color='aliceblue',
...
while True:
    ...
    if event == 'validar':
        palabra_validar = obtener_palabra_validar(window, limite_extremo_uno, limite_extremo_dos)

        if validar_palabra(palabra_validar):
            sg.Popup('La palabra {} es válida.\n'.format(palabra_validar),
                     title='¡Bien hecho!', font='Helvetica 11',
                     button_color=('white', 'green'))
        else:
            sg.Popup('La palabra {} es inválida.\n'.format(palabra_validar),
                     title='¡Vuelve a intentarlo!', font='Helvetica 11',
